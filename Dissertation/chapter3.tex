...
``Locate just enough facilities and associated capacity such that all demand is served within the capacity limits of each facility, given the coverage capabilities of each facility.''
...
В работе \cite{kontsevik2024connectivityPlacement} используется improved MCLP algorithm --- CLSCP-SO.
При этом cost matrix рассматривается как константа; в рассматриваемой постановке
cost matrix --- это матрица достижимости между выбранными районами,
...
\begin{table}[h]
    ...
    \begin{tabular}{c|cccccccccc}
        \textbf{block id} & \textbf{0} & \textbf{1} & \textbf{2} & \textbf{3} & \textbf{4} & \textbf{5} & \textbf{6} & \textbf{7} & \textbf{8} & \textbf{9} \\
        \hline
...
\min F(M),
\]
где fitness-функция в статье задана непосредственно как
\[
...
Transit Network Design Problem формулируется на расширенном городском графе
\[
C = (N, E_s, D),
\]
...
Здесь $C_p$ --- passenger cost, $C_o$ --- operator cost, $C_c$ --- штраф за нарушение ограничений.

Passenger cost задается как среднее внутрисетевое время поездки по всем парам спроса:
\[
C_p(C;R)=
...
\]
где $f_u$ --- доля пар origin--destination с положительным спросом, для которых в наборе маршрутов нет пути, а $f_l$ --- суммарное нарушение ограничений на длину маршрутов.
...
Weighted transport connectivity metric в статье записан следующим образом:
\[
C_w(C;R)=
\frac{1}{n}
\sum_{i=1}^{n}
